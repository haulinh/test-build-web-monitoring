...
export default class SelectStationAuto extends React.PureComponent {
  ...
  getStationAutos = () => {
    const { province, stationType, fieldFilter, fieldValue } = this.props
    ...
    const fieldFilterName = fieldFilter || fieldValue || 'key'

    if (province) {
      stationAutos = stationAutos.filter(stationAuto => {
        const provinceValue = _.get(
          stationAuto,
          ['province', fieldFilterName],
          ''
        )
    ...
    if (stationType) {
      stationAutos = stationAutos.filter(
        stationAuto => stationAuto.stationType[fieldFilterName] === stationType
      )
    }
