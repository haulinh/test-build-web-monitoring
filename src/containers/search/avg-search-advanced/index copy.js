...
export default class AvgSearch extends React.Component {
  ...
  handleSubmitSearch = searchFormData => {
    this.setState({ isSearchedData: true }, () => {
  ...
  handleOnSearchStationAuto = searchData => {
    this.setState({ isSearchStation: true }, async () => {
      const { data: stationKeys } = await DataStationAutoApi.searchStationAuto(
        searchData
      )
      this.setState({ stationKeys, isSearchStation: false })
    })
  }
  ...
  render() {
    console.log(this.state.isSearchedData)
    return (
      <PageContainer
          ...
          <Row type="flex">
            ...
            <Col span={this.props.isOpenNavigation ? 24 : 20}>
              ...
              <Spin
                size="large"
                tip="Searching..."
                spinning={this.state.isSearchStation}
              >
                <StationForm
