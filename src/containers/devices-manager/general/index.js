...
import Header from 'components/devices-manager/head'
import objectPath from 'object-path'
import HeaderFilter from 'components/devices-manager/filter'
import StationTypeList from 'components/devices-manager/station-type-group/station-type-list'
...
import {
  getGroupOptions,
  getOrderOptions
} from 'components/devices-manager/filter/options'
...
export const defaultFilter = {
  group: getGroupOptions()[0].value,
  order: getOrderOptions()[0].value,
  stationType: '',
  search: ''
}
...
@withRouter
@autobind
export default class MonitoringDevices extends React.Component {
  state = {
    isLoading: false,
    isLoadedFirst: false,
    filter: defaultFilter, //getMonitoringFilter() ? getMonitoringFilter() :
    data: []
  }

  appendWarningLevelStationAuto(stationAutoList) {
    return stationAutoList.map(stationAuto => {
      let totalWarning = { maintenance: 0, brokened: 0 }
      if (stationAuto.lastLog && stationAuto.lastLog.measuringLogs) {
        ...
        Object.keys(measuringLogs).forEach(key => {
          if (!measuringLogs[key].statusDevice || measuringLogs[key].statusDevice === 2) {
            totalWarning.brokened ++
          } else if (measuringLogs[key].statusDevice === 1) {
            totalWarning.maintenance ++
          }
        })
      }
      return {
  ...
  getTotalWarning(stationAutoList) {
    ...
    stationAutoList.forEach(item => {
      totalWarning.maintenance += item.totalWarning.maintenance
      totalWarning.brokened += item.totalWarning.brokened
    })
    return totalWarning
  }
  ...
  unGroupStation(stationTypeList) {
    ...
    if (this.state.filter.order === monitoringFilter.DEVICES.NAME) {
  ...
  getData() {
    ...
    // filter by brokened
    if (this.state.filter.order === monitoringFilter.DEVICES.BROKENED) {
      stationTypeList = this.sortNameList(stationTypeList, 'totalWarning.brokened').map(
        stationType => {
          return {
            ...stationType,
            stationType: stationType.stationType,
    ...
    // filter by brokened
    if (this.state.filter.order === monitoringFilter.DEVICES.MAINTENANCE) {
      stationTypeList = this.sortNameList(stationTypeList, 'totalWarning.maintenance').map(
        stationType => {
          return {
            ...stationType,
            stationType: stationType.stationType,
