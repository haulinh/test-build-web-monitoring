...
import {updateWarningLevelColorSensor} from 'redux/actions/config'
...
export default class WarningLevelColorOfSensor extends React.Component {
    ...
    return (
      <React.Fragment>
        <Table
          size="middle"
          pagination={false}
          dataSource={this.props.colorSensor.value}
  ...
  _saveConfigs = () => {
    ...
    validateFields(async (error, values) => {
      const id = _.get(this.props.colorSensor, '_id')
      const data = Object.values(values)
      this.setState({isSubmit: true})
      await this.props.updateWarningLevelColorSensor(id, data)
