...
export default class ReportType1 extends React.Component {
  ...
  handleSubmit(values){
    ...
    if(values.measuringList) measuringListStr = values.measuringList.map(item=>item.key).join(',')
    let url = getUrlReportType1(this.props.token, values.stationAuto, values.time.format("MM-YYYY"), measuringListStr)
