...
export default class QueryTab extends Component {
  ...
  getUrl = () => {
    ...
    const fieldsParams = _.get(data, 'config', []).map(field => ({
    ...
    const url = [
      dataRoutes.getPeriodicForecastNewest(),
      `id=${params.id}`,
    ].join('?')
  ...
  handleOnSearch = async () => {
    ...
    try {
      const data = await dataShareApiApi.getPeriodicForecastNewest(queryParams)
